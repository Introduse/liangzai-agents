...
description: >-
  ...
  "plugin-update".
area: Setup
use_for: "Idempotent catch-up after a plugin upgrade: re-check the gateway connector (including a STALE cached tool list), the gateway's Vault credentials, the six stalls, bowl definition, Cowork scheduled tasks, recipient allowlist, and the workspace CLAUDE.md embed; fill only what's missing."
---
...
| 2 | Connector is not **stale** | Compare the `liangzai_*` tools you can actually see against the tool table in `agents/liangzai.md`. The gateway advertises every tool in that table | A tool from the table is missing from your tool list. That means the connector cached an old tool list — **the fix is to reconnect it** (Settings → Connectors → remove `gateway`, add it again), NOT a change to the gateway. Never report a missing tool as a gateway gap without checking this first. After a plugin upgrade this is the FIRST thing to check: `liangzai_pending_documents` (newest, and it REPLACED `liangzai_logged_attachments` — seeing the old name is itself proof of a stale connector), `liangzai_daily_sales`, `liangzai_list_suppliers` and `liangzai_merge_suppliers` are all recent, and an old connector will simply not show them |
| 3 | Gateway credentials | **`liangzai_list_credentials`** — names only, never values. The gateway holds its own credentials in Vault now; nothing is sent on a call | `google_client_id`, `google_client_secret`, `gmail_refresh_token`, `supplier_mailbox`, `summary_recipients` or `loyverse_access_token` is missing from the list |
...
| Recipient allowlist | **liangzai-setup Step 3i** — ask who should receive the emails, then store it with **`liangzai_store_credential`** (`service: summary_recipients`). Write-only: it never reads a value back |
...
bowl definition. Then confirm in one
