...
|---|---|---|---|
| 1 | Gateway connector + key | **`liangzai_ping`** | `status` is anything other than `pong` — the connector isn't installed, or the key is wrong |
...
| 10 | **The token reads the RIGHT mailbox** | **`liangzai_ping`** — the same call as #1 — read `mailbox` (the address the stored Gmail token actually reads) against `supplier_mailbox`, and `mailbox_matches` | `mailbox_matches` is **`false`**. THIS IS THE ONE GAP EVERY OTHER CHECK PASSES THROUGH. #3 sees `gmail_refresh_token` present and is satisfied; #9 sees `last_poll` recent and is satisfied — because the poller *is* running, against somebody's personal inbox. `users/me` is whoever granted the token, and `supplier_mailbox` is only the From: address the summary sends as, so nothing else in this list can tell a configured system from a correct one. Between July and September 2026 that gap cost two months of supplier invoices: every poll ingested a Five Bucks mailbox, the classifier correctly rejected all of it as not-an-invoice, and month close reported "Everything matched" on an empty period. `null` means one side is unknown — treat it as unproven, never as agreement |
...
| Wrong mailbox (#10) | **liangzai-setup Steps 3g–3h, then 3j** — a genuine re-consent, and the one gap that is NOT filled by copying `.claude/settings.local.json`: the value there is present and wrong, so copying it re-installs the fault. Set `OAUTH_ACCOUNT` to the intended mailbox FIRST — the script's guard compares against it and refuses a sign-in as any other account, which is the whole protection. Then `google_oauth.py --auth-url`, he signs in **as the supplier mailbox** and pastes back the URL he lands on, then `--exchange "<url>"`, then store the printed token with `liangzai_store_credential` (`service: gmail_refresh_token`). If the OAuth **client** also changed, store `google_client_id` and `google_client_secret` in the same pass — a refresh token is only valid for the client that minted it, and a new token under an old client fails as `unauthorized_client`. Confirm with `liangzai_ping` (`mailbox_matches: true`) before anything else |
